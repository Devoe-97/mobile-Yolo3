...
def ConvertVOCXml(file_path="",file_name="",img_base_path=""):
   tree = ET.parse(file_name)
   root = tree.getroot()
   # print(root.tag)

   img_base_path=img_base_path+'\\'+root.attrib["name"]

   #创建文件夹
   file_path=file_path+'\\'+root.attrib["name"]
   if os.path.exists(file_path) == False:  # 判断文件夹是否存在
   ...
   for child in root:
      ...
      if(child.tag=="frame"):
         ...
         output_file_name = "img" + pic_id + ".xml"
      #  print(output_file_name)

         folder = doc.createElement("folder")
         #folder.appendChild(doc.createTextNode("VOC2007"))###########################
         folder.appendChild(doc.createTextNode(root.attrib["name"]))
         ...
         # pic_name=root.attrib["name"]+"__img"+pic_id+".jpg"########################
         pic_path = img_base_path+'\\'+"img" + pic_id + ".jpg"
         path.appendChild(doc.createTextNode(pic_path))
...
def bboxes_draw_on_img(img, bbox, color=[255, 0, 0], thickness=1):
    ...
    print(bbox)
    if bbox["name"]=="car":
        cv2.rectangle(img, p1, p2, [255, 0, 0], thickness)
        cv2.putText(img, str(bbox["name"]) ,(int(float(bbox["xmin"]) + 5), int(float(bbox["ymin"]) - 7)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,[255, 0, 0], 1)
    elif bbox["name"]=="bus":
        cv2.rectangle(img, p1, p2, [0, 255, 0], thickness)
        cv2.putText(img, str(bbox["name"]), (int(float(bbox["xmin"]) + 5), int(float(bbox["ymin"]) - 7)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,[0, 255, 0], 1)
    elif bbox["name"]=="van":
        cv2.rectangle(img, p1, p2, [0, 0, 255], thickness)
        cv2.putText(img, str(bbox["name"]), (int(float(bbox["xmin"]) + 5), int(float(bbox["ymin"]) - 7)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,[0, 0, 255], 1)
...
if ( __name__ == "__main__"):
    '''
    #print("main")
    basePath="D:\MachineLearning11\\UA-DETRAC\\DETRAC-Train-Annotations-XML\DETRAC-Train-Annotations-XML"
    img_base_path="D:\MachineLearning11\\UA-DETRAC\\DETRAC-train-data\Insight-MVT_Annotation_Train"
    totalxml=os.listdir(basePath)
    total_num=0
    flag=False
    print("正在转换")
    saveBasePath="D:\MachineLearning11\\UA-DETRAC\\DETRAC-VOC"
    ...
    for xml in totalxml:
        ...
        print(file_name)
        num=ConvertVOCXml(file_path=saveBasePath,file_name=file_name,img_base_path=img_base_path)
        print(num)
    ...
    '''
    visualization_image("D:\MachineLearning11\\UA-DETRAC\\DETRAC-train-data\Insight-MVT_Annotation_Train\MVI_20012/img00001.jpg","D:\MachineLearning11\\UA-DETRAC\\DETRAC-VOC\MVI_20012\img00001.xml")
